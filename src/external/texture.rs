...
impl Texture {
    ...
    pub fn lambertian(c: RGB) -> Self {
        Self { contents: internal::Texture::Lambertian(c.to_internal()) }
    }

    #[staticmethod]
    pub fn metal(c: RGB, fuzzy: f64) -> Self {
        Self { contents: internal::Texture::Metal(c.to_internal(), fuzzy) }
    }

    #[staticmethod]
    pub fn light(c: RGB) -> Self {
        Self { contents: internal::Texture::Light(c.to_internal()) }
    }

    #[staticmethod]
    pub fn dielectric(c: RGB, idx: f64) -> Self {
        Self { contents: internal::Texture::Dielectric(c.to_internal(), idx) }
    }
}
